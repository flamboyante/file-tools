...
class Ycyk_422_Work():
    ...
    def __init__(self, id: int):
        ...
        self.id = id                # 标识符
        self.file_count = 0         # 文件型传输序列计数
        self.cmd_count = 0          # 控制指令和参数注入型指令源包序列计数
        self.frame_size = 1024      # 定义帧的大小，单位可能是字节
        self.frames = 1024         # 定义帧的数量
        self.segment_size = self.frames * self.frame_size   # 计算总的段大小，等于帧的数量乘以帧大小
        self.segments = 0           # 初始化段计数器
        self.segments_end = 0       # 初始化段结束长度
        self.segments_end_frames = 0
        self.if_file_divide = 0     # 是否分段, 0-不分段, 其他-分段
    ...
    def check_apid(self, array):
        if self.flash is 0xFA:
            self.apid = 0x1D
        elif self.flash is 0xFB:
            self.apid = 0x18
        elif self.flash is 0x9b:
            self.apid = 0x18
    ...
    def refactor_begin(self, apid, flash, mem):
        self.flash = flash

        packet = bytearray()
        packet.append((self.id >> 8) & 0xFF)
        packet.append(self.id & 0xFF)
        packet.append((apid >> 4) & 0x0F)
        packet.append((apid << 4) & 0xF0)
        packet.append(0xC0 | (self.cmd_count >> 8 & 0x3f))
        packet.append(self.cmd_count & 0xff)
        packet.extend([0x00, 0x03, 0x01, 0xAF])
        packet.append(flash)
        packet.append(mem)
        accumulator = self.reverse_accumulate(packet)
        packet.append((accumulator >> 8) & 0xFF)
        packet.append(accumulator & 0xFF)

        self.cmd_count = (self.cmd_count + 1) & 0x3FFF

        self.binary_print_hex(packet)
        return packet

    def get_frame_segment(self, file_length_all):

        if self.if_file_divide is 0:
            self.frames = int(file_length_all / self.frame_size)
            if file_length_all % self.frame_size != 0:
                self.frames += 1
            self.segments_end_frames = self.frames
            self.segments = 1
            self.segments_end = file_length_all
            self.file_seg_length = file_length_all
        else:
            self.segment_size = self.frame_size * self.frames
    ...
    def send_begin(self, apid, file_path, flash, mem, divide, frame_len, frame_num):

        self.if_file_divide = divide
        ...
        packet.append(0xC0 | (self.cmd_count >> 8 & 0x3f))
        packet.append(self.cmd_count & 0xff)
        packet.extend([0x00, 0x10, 0x01, 0x55])
        packet.append(apid)
        packet.append(flash)
        packet.append(mem)
        packet.append(((self.if_file_divide & 0x3) << 6) | ((self.segments >> 8) & 0x3F))
        packet.append(self.segments & 0xFF)
        packet.append((self.file_seg_length >> 24) & 0xFF)
        packet.append((self.file_seg_length >> 16) & 0xFF)
        packet.append((self.file_seg_length >> 8) & 0xFF)
        packet.append(self.file_seg_length & 0xFF)
        packet.append((self.segments_end >> 24) & 0xFF)
        packet.append((self.segments_end >> 16) & 0xFF)
        packet.append((self.segments_end >> 8) & 0xFF)
        packet.append(self.segments_end & 0xFF)
        packet.append((crc_return >> 8) & 0xFF)
        packet.append(crc_return & 0xFF)
    ...
    def send_datas(self, apid, seg, group_flag, file_data):
        if group_flag is 1:
            self.file_count = 0
        ...
        packet.append(self.id & 0xFF)
        packet.append((apid >> 4) & 0x0F)
        packet.append(((apid << 4) & 0xF0) | 0x0F)
        packet.append(((group_flag & 0x3) << 6) | ((self.file_count >> 8) & 0x3F))
        packet.append(self.file_count & 0xFF)
        packet.append((packet_len >> 8) & 0xFF)
        packet.append(packet_len & 0xFF)
        packet.append((seg >> 8) & 0xFF)
        packet.append(seg & 0xFF)
        ...
        self.file_count = (self.file_count + 1) & 0x3FFF
    ...
    def send_finish(self, apid):
        ...
        packet.append(0xC0 | ((self.cmd_count >> 8) & 0x3F))
        packet.append(self.cmd_count & 0xFF)
        packet.extend([0x00, 0x01, 0x01, 0xAA])
        accumulator = self.reverse_accumulate(packet)
        packet.append((accumulator >> 8) & 0xFF)
        packet.append(accumulator & 0xFF)
