# -*- coding: utf-8 -*-
"""can_commands · CAN 指令模型与预设（纯数据，不依赖 Qt）。

来源：`WorkClass/CANCommandScheduler.py`（生产实现，2026-09-21 迁入 gui-ng）。
搬运时保持逐字语义不变——里面的帧值、预设、ID 拼装都被真机验证过，不要"优化"。

指令模型：一条 `CanCommand` = 用户视角一行。
- 变长由 `frames` 决定（每帧一个外部给好的完整 29bit ID + ≤8B payload）
- 定时发送由单一心跳到期表驱动，支持每条独立间隔（>=10ms）与次数（<=0 无限）
"""
import copy
...
# --------------------------------------------------------------------------
# 预设库（沿用旧 CanWindow 的固定测试指令；ID 拼装方式与 ycyk_422 一致）
# --------------------------------------------------------------------------
...
def default_commands(channel):
    """某通道的出厂预设（深拷贝，避免与模块常量共享 frame 对象）。"""
    src = PRESETS_A if channel == 'A' else PRESETS_B
    return [c.copy() for c in src]


# --------------------------------------------------------------------------
# 调度器：单心跳 + 到期表（发送动作由外部注入回调）
# --------------------------------------------------------------------------
class CANCommandScheduler(object):
    """纯 Python 调度器，不依赖 Qt 线程；由外部以固定节拍调用 tick()。"""

    def __init__(self):
        self._entries = {}       # id(cmd) -> cmd（同一份对象引用，供 UI 展示状态）
        self._last = {}          # id -> monotonic 上次发送时刻
        self._remaining = {}     # id -> 剩余次数（对有限指令）
        self._on_due = None      # callable(cmd)
        self._just_finished = [] # 本次 tick 内"次数用尽自动停"的 id 列表
