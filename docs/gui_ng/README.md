...
| SC 422（SC 天线） | 921600 | 8 | **Odd** | 1 | 当前跑文件传输；也是 sc422 页默认 |
| CAN A / B | 500K | — | — | — | 蒋氏 USBCAN |

ConnectionBar v2 已把这四个固化成**预设**（选预设自动套波特率 + 校验）：

```
预设 [SC 422 ▾]  端口 [COM10 ▾] ⟳  波特率 [921600 ✎]  校验 [奇校验 (O) ▾]  [打开]  [●已连接]
```
...
### A. 控制台（debug 口，如 COM4，115200 8N1）
...
### B. 文件传输（SC 422，COM10，921600 8O1）
...
- [ ] 发心跳帧 `EB 90 01 80 C0 00 00 01 00 1D FE A0`，看是否回 `1A CF ... 00 1F ...`
      （注意：心跳原属 **self 422**；接 SC 口时按实际业务发对应帧）
