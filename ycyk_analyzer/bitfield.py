...
单独成文件的理由：位序出错会让所有数值都错、而且不报错，隔离出来只做一件事 ——
从字节流里取一段二进制位，转成整数。越界直接抛错，不静默返回 0 或垃圾值。

位序约定：高位先（MSB-first）—— 一个字节里，最左边（最高位）先算。

    例：字节 0x03, 0xF2 拼成 16 位，值 = 0x03F2 = 1010
        （慢遥 slot2 包里第一条消息的"消息ID"）
"""
