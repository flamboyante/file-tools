...
T 段和 Z 段完全不同 —— 它**不需要位域解码**：
    CSV 里 T 段的值已经是解码好的中文枚举或数值（例如「默认AB同时工作」、「2」），
    所以 T 段只需做一件事：**按列序 1:1 贴到表里的行上**，拿到中文名和判据说明。

    CSV 快遥  68 列 `TMXZJDT1xxx`  ↔  合并表「标准快遥」    68 行
    CSV 慢遥 111 列 `TMXZJDT2xxx`  ↔  合并表「标准慢遥51H」 111 行

★ 映射靠"**行序**"而不是"名字匹配"（CSV 的列名里只有序号、没有中文）。
  所以**列数必须精确相等** —— 对不上直接报错，绝不硬猜：
  列错位一位，整张表的列名就全错，而且不会报错（这正是外部工具栽跟头的地方）。

★ 提醒：T 段表第 3 列的**列名**叫「转换公式」，但**内容**是枚举说明与判据
  （例如 `0000：正常; 1111：异常`）—— 别因为列名是"公式"就把这列跳过。

用法：
    python t_segment.py --spec                  打印两份 T 段定义（列名 + 判据说明）
    python t_segment.py <csv文件> <行号>         打印该行的 T 段映射结果
"""
