# -*- coding: utf-8 -*-
"""
spec.py —— 把解析表读成 Python 对象（SlotSpec / MessageSpec / FieldSpec）。

只负责读表：不碰 CSV、不解析数据、不管判据。

表的组织方式：
    每页 sheet 叫「通道N（slotM）」，一页 = 一个 slot 的完整帧定义（合计 1184 bit = 148 字节）。

        第 1 行    表头
        然后        以「消息ID」为界切分成若干"消息"，每条消息含：
                      消息ID行（新消息开始，2 字节）
                      消息长度行（备注里写着 length=NN字节）
                      若干字段行
        末尾        校验行：参数名称为空、长度bit = 1184

    切分规则：参数名称里出现"消息ID"，即一条新消息的开头。

读表要处理的三件事：
    1. 消息ID 有三种写法，type 必须"先看名称、再兜底看备注"：
          消息ID（type=1000）      type 写在参数名称里
          消息ID                  裸的，type 写在"备注"里（type=1010）
          BMU消息ID(t=1050)       用 t= 而不是 type=
    2. 一条消息可能对应两个 type（按制式分）：备注写 type=1200(sc),1201(ka) → type 存成列表。
    3. 消息之间夹着"预留 / 保留 / 慢遥帧序号"这类不属于消息内容的行；
       若算进消息，位长与备注声明的 length 对不上 → 每行标一个 role
       （header / field / padding / trailer）分开统计。
"""
...
# 备注里的"多 type"写法要整体匹配，例如 type=1200(sc),1201(ka)：
# 逗号后面那个数字前没有 type= 前缀，只按单个数字匹配会漏掉它。
...
def default_spec_path() -> str:
    ...
    由 `tools/merge_spec_tables.py` 生成。

    查找顺序（第一个存在的即为结果）：
      1. 环境变量 `YCYK_SPEC` 指定的文件 —— 明确指定时不再回退，找不到即报错
      2. `程序目录/spec/spec_merged_*.xlsx` —— 外置表，改判据不必重新打包
