# -*- coding: utf-8 -*-
"""
frame.py —— 把一个 slot 包（148 字节）解析成"消息 + 字段值"，并同时自证解析正确。

一、定位消息（两套机制并用）
    1. 游标扫描：从偏移 0 开始 ——
           读 2 字节"消息ID" → 读 2 字节"消息长度" → 按定义解码 → 跳到 (4 + 长度) 字节之后
       遇到连续 0（空槽）跳过；查到表里没有的 ID 立刻报错（不猜、不跳过）。
       消息不能按固定偏移取：表里的消息顺序 ≠ 包里的实际顺序
       （表写 1000→1100→1101→1800→1052，包里是 1000→1800→1100→1101→1052），
       表只能当"消息字典"用。
    2. 帧尾锚定：BMU 的三个消息（1050/1051/1052）按"从帧尾倒推"定位 ——
           起始偏移 = 147 − (消息总位长 − 帧尾位长) / 8
       对应表里"BMU 上报遥测按固定位置解析"的说法。

二、自证（不依赖任何外部工具）
    每个包做以下几项校验，全部通过才认为解析可信：
        · 包长 = 148 字节
        · 帧尾序号 → 对应的 slot 与正在用的表页一致
        · 游标读到的消息ID 能查到定义（查不到 = 定位错了）
        · 游标读到的长度 = 表里声明的 length
        · 锚定消息解出来的"消息ID / 消息长度"两个头字段 = 表里定义
    自证比与外部工具产物对拍更严格：它证明的是"位置找对了"，而不是"值一样"。
"""
...
def walk_cursor(
    ...
    关于"扫不动了"的判定：
        ...
        （很多包里该 slot 的前若干字节是 0 或 0x5A 填充）。
        所以扫到识别不了的内容时，只记录停在哪里，不算自证失败 ——
        真正参与判定的是帧尾锚定的 BMU 消息（位置固定、每拍都应该有）。
