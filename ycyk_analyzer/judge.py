...
判据住在合并解析表（`spec/spec_merged_*.xlsx`）里，不在代码里。
格式由 `tools/normalize_judges.py` 归一，共 8 种：

    正常值:X[,级:异常|告警]      值等于 X 即通过（多值用 | 分隔；支持 0x/0b/FF/中文枚举）
    正常范围:a~b[,级:...]        值落在 [a,b] 内即通过
    文本:词|词[,级:...]          值含这些词即命中（用于"值本身就是中文结论"的字段）
    结构对应:帧序号=N            该消息必须出现在帧序号=N 的包里（进 frame 自证，不逐拍判）
    总体:有变化 / 总体:按历史基线  跨拍判据（当前未启用，保留语义）
    无需判据 / 待定               不判 → 「暂不判据」

判据只描述"什么算正常"，命中不了就是问题；严重度（异常 / 告警）由 `,级:` 给。

两种判定：
    judge(...)      —— 瞬时（逐拍）：给一个值，返回 通过 / 告警 / 异常 / 暂不判据
    summarize(...)  —— 总体（跨拍）：给"异常拍数 / 总拍数"，返回一句总体结论

openpyxl 读不了时（文件被其他工具重写过样式）自动退回解析 xlsx 内部 XML。
"""
...
# 含义部分不能含冒号 —— 否则「1：正常：0：异常」会被贪婪匹配吞成一条
...
def to_number(text: str) -> Optional[float]:
    ...
    数字解析要覆盖的写法：
      · 「0b」「1b」这种只有标记没有数字的（表里大量出现）→ 按二进制 0 / 1
        （不能落到十六进制分支，否则 "0b" 会被解析成 11）
      · 裸写的 `31` 一律按十进制（不是十六进制）
      · 纯十六进制字符（FF）才按十六进制兜底
    """
    ...
    def evaluate(self, value) -> str:
        ...
        if self.kind == KIND_VALUE:
            if not text:
                return NA
            got = to_number(text)
            # 值不是数字时（快遥 T 段的值是解码后的中文，而判据那格写的是位模式），
            # 按下面三步判；关键是第 ② 步「查备注映射」：
            if got is None:
